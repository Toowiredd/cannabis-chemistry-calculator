      ...
      <div className="flex flex-col gap-4 px-1 py-2 sm:px-2 sm:py-3">
        ...
        {step.id === 'product-type' ? (
          <EndProductCoverflow
            ...
            onSelect={endProductId => onConfirm(endProductId)}
          />
        ) : step.renderCustom ? (
          // Custom-input step (e.g. the future Name step's
          // typed-name form). The step's `renderCustom`
          // callback owns its own form widgets + confirm
          // CTA; the StepCard just routes the props through.
